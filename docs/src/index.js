...
_exports.style=style;var mdcButtonCss={style:style};_exports.$mdcButtonCss=mdcButtonCss;class SSButton extends LitElement{static get properties(){return{disabled:{type:Boolean,attribute:"disabled",reflect:!0},label:{type:String,attribute:"label",reflect:!0},icon:{type:String,attribute:"icon",reflect:!0}}}constructor(){super();this.disabled=!1;this.label="";this.icon=""}static get styles(){return[GlobalStyles,style,css`
    ...
    `}firstUpdated(){this._verifySession()}_authenticate(){const url="https://accounts.spotify.com/authorize?response_type=token&",clientId=config.SPOTIFY_CLIENT_ID,redirectUri="https://jmcgavin.github.io/SpotifyScanner/callback",scope="user-read-private playlist-modify-private",state=generateRandomString(16);localStorage.setItem(this._stateKey,state);const queryString=createQueryString({client_id:clientId,scope,redirect_uri:redirectUri,state// show_dialog: 'true'
